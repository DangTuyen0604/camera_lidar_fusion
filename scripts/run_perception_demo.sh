...
export ROS_LOG_DIR="${ROS_LOG_DIR:-/tmp/camera_lidar_fusion_perception_logs}"
mkdir -p "$ROS_LOG_DIR"
...
if [[ ! -f "$project_root/install/setup.bash" ]]; then
  echo 'Workspace is not built. Run: ./scripts/build_workspace.sh' >&2
  exit 1
fi
...
set -u
ros2 launch fusion_bringup perception_demo.launch.py "$@"
